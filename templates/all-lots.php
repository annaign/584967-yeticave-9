<div class="container">
    <section class="lots">
        <h2>Все лоты в категории «<span><?= htmlspecialchars($category_search['category_title']) ?></span>»</h2>
    ...
    <?php if (isset($pages) && $pages > 1) : ?>
        <ul class="pagination-list">
            <?php $page_href = '/all-lots.php?category=' . htmlspecialchars($category_search['id']) . '&page=' ?>
            ...
            <?php for ($page = 1; $page <= $pages; $page++) : ?>
                <li class="pagination-item <?= $page === $current_page ? 'pagination-item-active' : '' ?>">
                    <?php if ($page !== $current_page) : ?>
                        <a href="<?= $page_href . $page . '&limit=' . htmlspecialchars($items_on_page) ?>"><?= htmlspecialchars($page) ?></a>
                    <?php else : ?>
                        <a><?= htmlspecialchars($page) ?></a>
                    <?php endif ?>
                </li>
            <?php endfor ?>
